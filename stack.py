...
# nn_model.fit(X_train_stack, y_train)
# preds = nn_model.predict(X_test_stack)
# rmse = mean_squared_error(y_test, preds)**0.5
# print('xgbr RMSE:', rmse)
# print(nn_model.best_params_)
# preds_test = nn_model.predict(test_stack.loc[:,X_train_stack.columns])
# preds_test = np.expm1(preds_test)
# submiss = pd.DataFrame({'Id': test['Id'], 'SalePrice': preds_test})
...
#     # create model
#     model = Sequential()
#     model.add(Dense(13, input_dim=12, kernel_initializer='normal', activation='relu'))
...
# estimator = KerasRegressor(build_fn=larger_model,
#                            epochs=1000,
#                            batch_size=5,
#                            verbose=1,
#                            callbacks=[earlyStopping])
# estimator.fit(X_train_stack, y_train)
...
# preds_test = lasso_model.predict(test_stack.loc[:,X_train_stack.columns])
# preds_test = np.expm1(preds_test)
# submiss = pd.DataFrame({'Id': test['Id'], 'SalePrice': preds_test})
